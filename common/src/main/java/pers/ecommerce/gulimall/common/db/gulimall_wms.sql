...
(
  ...
  DEFAULT CHARSET = utf8 COMMENT ='日志';
...
(
    ...
    `status`      int(11)        DEFAULT NULL COMMENT '状态：[0: 新建; 1: 已分配; 2: 正在采购; 3: 已完成，4: 采购失败]',
    creator       bigint comment '记录创建者',
    create_date   date comment '记录创建时间',
    update_date   date comment '记录修改时间',
    PRIMARY KEY (`id`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4 COMMENT ='采购详细信息';
...
(
    ...
    `ware_id`     bigint(20)   DEFAULT NULL COMMENT '仓库id',
    `lock_status` int(1)       DEFAULT NULL COMMENT '锁定状态：[1: 已锁定; 2: 已解锁; 3: 扣减]',
    creator       bigint comment '记录创建者',
    create_date   date comment '记录创建时间',
    update_date   date comment '记录修改时间',
    PRIMARY KEY (`id`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4 COMMENT ='库存工作单详细信息';
